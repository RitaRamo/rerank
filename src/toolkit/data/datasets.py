...
class CaptionTrainContextLSTMRetrievalDataset(CaptionDataset):
    ...
    def __init__(self, dataset_splits_dir, features_fn, normalize=None, features_scale_factor=1, max_caption_len=52):
        super().__init__(dataset_splits_dir, features_fn,
                         normalize, features_scale_factor)
    ...
    def train_retrieval(self, train_dataloader_images):
        ...
        for (images, contexts, decode_lengths, targets) in tqdm(train_dataloader_images):
            ...
            if start_training:
                ...
                _, _, extras = self.context_model(images, contexts, decode_lengths, teacher_forcing, device="cpu")
...
def get_context_lstm_retrieval(create, context_model, retrieval_data_loader=None):
    ...
    if create:
        ...
        for i, (images, contexts, decode_lengths, targets) in enumerate(retrieval_data_loader):
            ...
            nearest_targets, distances=image_retrieval.retrieve_nearest_for_train_query(hidden_state.detach().numpy())
